...
1. 保留完整需求：将当前问题原样传给 `recommend_products`，不要删除品类、预算、品牌、场景、肤质、使用感、偏好或避雷条件。
2. 选择数量：用户明确要求数量时，将其作为 `limit`；否则使用工具默认值。不要为了凑数量扩大到无关品类。
3. 统一调用：文本、纯图片和图文任务都只调用 `recommend_products` 一次。图片由运行时自动传入，不要自行读取图片 URL 或调用底层检索。
4. 等待工具完成内部流程：需求解析、候选召回、真实数据校验、候选语义审核、基础偏好软排序和商品卡构造均由工具内部负责。不要在 Agent 层重复这些步骤。
5. 按 `action` 收尾：
   - `recommend`：只介绍 `product_cards` 中的商品，并使用 `ranked_products`、`match_status`、`constraint_gaps` 和已有理由解释选择。
   - `clarify`：自然提出 `clarify_question`，然后停止，不要自行补全需求后重搜。
   - `empty`：如实说明 `empty_reason`，然后停止，不要改用其他商品工具凑答案。
6. 拿到任一结果后立即回答。不要改写 query 再搜一次，不要自行增删候选、重排、补卡或创造商品。

## 受控 scripts

当前 `recommend_products` 已返回最终 `product_cards` 时，不再运行 script。只有 ToolResult 明确提供尚未终结的结构化数据或要求确定性处理时，才调用 `run_shopping_skill_script`：

- `normalize-candidates`：统一候选字段别名；输入 `{"candidates":[...]}`。
- `validate-selection`：提交候选编号前做原子校验；输入 `{"candidates":[...],"selected_indices":[1,2]}`。
...
- 只有候选中没有相关商品时才是 `empty`。工具已经完成候选判断，Agent 不再自行审核或改变标记。
