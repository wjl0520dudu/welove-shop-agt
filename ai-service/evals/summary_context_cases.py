...
Every fixture contains a continuous, visible user/assistant conversation.  The
runner expands its *old* prefix to a fixed size, then verifies three follow-up
turns after the same summary has been generated once and reused.
"""
...
def _messages(topic: str, facts: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    old_prefix = [
        ...
        {"role": "user", "content": "先不用立刻下结论，我再补充一些背景。"},
...
def _continuations(topic: str, facts: str, first_question: str, route: str) -> list[dict[str, str]]:
    if route == "shopping":
        ...
        questions = [first_question, "把刚才讨论过的重点再归纳一下。", "谢谢，今天先这样。"]
    return [
        {
            "question": question,
...
def summary_context_cases() -> list[dict[str, Any]]:
    ...
    specs = [
        ("summary-001", "通勤耳机", "预算500元以内，通勤使用，优先降噪和舒适佩戴", "按我前面说的预算和通勤场景推荐耳机", "shopping"),
        ("summary-002", "油皮防晒", "油皮，夏天出油多，想要清爽不黏腻的防晒", "继续按我之前的肤质推荐防晒", "shopping"),
        ("summary-003", "护肤成分", "此前讨论了烟酰胺、视黄醇及其刺激风险", "我之前问过哪些成分问题？", "chitchat"),
        ("summary-004", "跑鞋", "男士通勤慢跑，预算800元以内，需要缓震和日常耐穿", "按前面的条件推荐跑鞋", "shopping"),
        ...
        ("summary-008", "话题切换", "早先讨论油皮护肤；当前只想了解开放式耳机和入耳式耳机区别", "开放式耳机和入耳式耳机有什么区别？", "knowledge"),
        ("summary-009", "手机", "预算5000元以内，重视拍照、续航和通勤使用", "按刚才的预算推荐手机", "shopping"),
        ("summary-010", "敏感肌面霜", "敏感肌，想保湿修护，避免刺激性成分和浓重香味", "前面说的敏感肌面霜怎么选？", "shopping"),
        ("summary-011", "商品对比", "此前推荐三款真无线耳机，重点是降噪、续航和佩戴", "把刚才那几款耳机按降噪和续航对比一下", "shopping"),
        ("summary-012", "防晒知识", "已经讨论防晒补涂频率、通勤场景和油皮肤感", "防晒霜多久补涂一次？", "knowledge"),
        ("summary-013", "护肤回顾", "用户依次问了视黄醇、果酸、烟酰胺的作用和搭配", "我刚才问了什么？", "chitchat"),
        ("summary-014", "运动鞋", "用户需要男士42码、周末跑步和日常通勤都能穿的跑鞋", "继续推荐适合我的跑鞋", "shopping"),
        ...
        ("summary-017", "对话收尾", "用户已得到油皮防晒推荐，并表示暂时不需要更多商品", "谢谢，今天先这样", "chitchat"),
        ("summary-018", "知识话题切换", "前面讨论了咖啡推荐；当前问题转为透明质酸的保湿原理", "透明质酸为什么能保湿？", "knowledge"),
        ("summary-019", "预算继承", "用户先设定300元预算，再要求送给妈妈、偏温和护肤", "继续在之前预算内推荐礼物", "shopping"),
