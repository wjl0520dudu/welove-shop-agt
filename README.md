...
以下内容属于后续演进方向，不影响当前稳定演示版的核心链路。

### 1. 公共接口与 AI 导购防滥用

未来可以继续完善商品数据接口和 AI 导购入口的防刷能力：商品列表、详情、搜索等公共读接口按客户端、用户和接口维度进行分级限流；AI 导购按用户、IP、会话和时间窗口控制请求频率，并结合并发上限、配额、熔断和降级保护 LLM 成本。生产环境可由 CDN/WAF、Gateway、Redis/Sentinel 和业务服务分层承担不同职责，同时补充测试账号清理、OSS 图片访问保护和异常告警。

### 2. Router 的动态能力发现

当前 Router 的下游类型仍主要由结构化 Prompt 和代码分支描述。后续可以建立 Agent/Skill 能力注册表，由下游 Agent 声明名称、职责、输入输出契约、可用工具和适用条件，Router 在运行时读取能力目录或版本化 Skill Catalog，自动发现可路由节点，减少新增 Agent 时修改 Router Prompt 和硬编码分支的范围。

### 3. Skills 生命周期与权限治理

随着 Skill 数量增加，需要补充 Skill manifest、版本、依赖、工具权限、输入输出 Schema 和兼容性检查，并在启动或发布阶段执行 Skill 校验。这样可以避免 Skill 之间的职责重叠、工具越权和升级后的隐性回归。

### 4. 评测、可观测性与成本分析

后续重点观察评测指标，增强可观察性与成本分析，可将按三类指标形成统一的版本评测报告：质量指标评估路由准确率、上下文指代、商品相关性、约束满足、知识正确性、工具选择和复杂任务完成情况；性能指标评估首个可见 Token 时间（TTFT）以及 P50/P95 延迟；成本指标统计输入、输出和总 Token，并进一步按模型价格换算请求成本。后续版本确保必须使用同一份 Golden Dataset、相同的评测配置和一致的统计口径，才能进行可重复的横向对比。

### 5. 长会话与并发可靠性

继续增强会话摘要水位、Checkpointer、持久化消息、重生成替换链和同会话并发锁的一致性，重点覆盖长对话、重复发送、断线重连、停止生成、重试和多端同时访问等场景。

### 6. 检索与多模态质量

后续可以优化 RAG 分块、召回与重排策略，增加证据新鲜度和来源可信度控制，并持续完善图文冲突、视觉模型不确定、纯图检索和商品卡真实性校验，避免模型表达超过检索证据。
